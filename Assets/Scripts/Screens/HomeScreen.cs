...
public class HomeScreen : PanelBase
{
    ...
    private IEnumerator UpdateDose(string value)
    {
        ...
        using (UnityWebRequest www = UnityWebRequest.Post("http://39.107.240.174/api/sober/input?appid=sober&access_token=0000&sign=12345", form))
        {
    ...
    private IEnumerator GetDoseData()
    {
        ...
        using (UnityWebRequest www = UnityWebRequest.Post("http://39.107.240.174/api/sober/getdose?appid=sober&access_token=0000&sign=12345", form))
        {
            yield return www.SendWebRequest();
