...
public class ChatScreen : MonoBehaviour
{
    ...
    void IntializeList()
    {
        for (int resultIndex = 0; resultIndex < Chatmanager.Instance.chatHistrory.Count; resultIndex++)
        {
            if (Chatmanager.Instance.chatHistrory[resultIndex] is UserMessage)
            {
                ...
                {
                    temp.GetComponent<ChatBubble>().Initialize(((UserMessage)Chatmanager.Instance.chatHistrory[resultIndex]).Sender.Nickname, ((UserMessage)Chatmanager.Instance.chatHistrory[resultIndex]).Message, MessageType.Owner);
                }
                else
                {
                    temp.GetComponent<ChatBubble>().Initialize(((UserMessage)Chatmanager.Instance.chatHistrory[resultIndex]).Sender.Nickname, ((UserMessage)Chatmanager.Instance.chatHistrory[resultIndex]).Message, MessageType.Friend);
                }
            }
        }
    }
    ...
    public void AddChatMessage(BaseMessage message)
    {
        if (message is UserMessage)
        {
            ...
            {
                temp.GetComponent<ChatBubble>().Initialize(((UserMessage)message).Sender.Nickname, ((UserMessage)message).Message, MessageType.Owner);
            }
            else
            {
                temp.GetComponent<ChatBubble>().Initialize(((UserMessage)message).Sender.Nickname, ((UserMessage)message).Message, MessageType.Friend);
            }
        }
    }
